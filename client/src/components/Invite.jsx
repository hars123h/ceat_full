...
const Invite = () => {
    ...
    if (loading || userDetails === null) {
        return (
            <div className=' bg-[rgb(164,29,35)]  flex flex-col text-white font-light p-5 relative'>
                ...
                <div className="info  sm:text-xs md:text-md flex flex-col gap-2 rounded-lg bg-white mt-5">
                    <div className='text-left bg-[rgb(164,29,35)] font-semibold rounded-t-lg text-white text-lg border-b pl-2 py-2 border-red-800'>Invitation Rewards:</div>
                ...
                <div className="flex gap-2 mt-[40px]">
                    ...
                    <div className="info w-1/2 p-3 sm:text-xs md:text-md flex flex-col rounded-2xl bg-white text-black font-bold mt-5">
                        ...
                        <CopyToClipboard text={` `} onCopy={() => toaster('copy success')}>
                            <span className='w-[80px] text-sm text-center bg-[rgb(164,29,35)] text-white p-2'>copy</span>
                        </CopyToClipboard>
                    </div>
    ...
    return (
        <div className=' bg-[rgb(164,29,35)]  flex flex-col text-white font-light p-5 relative'>
            ...
            <div className="info  sm:text-xs md:text-md flex flex-col gap-2 rounded-lg bg-white mt-5">
                <div className='text-left bg-[rgb(164,29,35)] font-semibold rounded-t-lg text-white text-lg border-b pl-2 py-2 border-red-800'>Invitation Rewards:</div>
                <span className='p-2 text-black font-semibold text-sm'>
                    Level 1 = <span className='text-red-600'> 15%</span>
                    <br />
            ...
            <div className="flex gap-2 mt-[40px]">
                ...
                <div className="info w-1/2 p-3 sm:text-xs md:text-md flex flex-col rounded-2xl bg-white text-black font-bold mt-5">
                    <div className='font-bold text-sm'>Invitation Link</div>
                    <div className='py-1 px-1 text-xs rounded-md border overflow-hidden border-red-800 mt-2'>{`${origin}/register/invite_code/${userDetails.user_invite}`}</div>
                    <CopyToClipboard text={`${origin}/register/invite_code/${userDetails.user_invite}`} onCopy={() => toaster('copy success')}>
                        <span className='w-[80px] text-sm text-center bg-[rgb(164,29,35)] text-white p-2'>copy</span>
                    </CopyToClipboard>
                </div>
            ...
            <div className="qr mx-auto flex flex-col justify-center items-center mt-1 p-1 bg-[rgb(164,29,35)] rounded-md">
                <QRCode
                    size={140}
                    style={{ height: "auto", maxWidth: "100%", width: "100%" }}
                    value={`${origin}/register/invite_code/${userDetails.user_invite}`}
                    viewBox={`0 0 120 120`}
                />
